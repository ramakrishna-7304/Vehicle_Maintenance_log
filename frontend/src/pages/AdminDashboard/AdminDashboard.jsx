    ...
    <div className="h-screen w-full bg-gray-900 text-gray-100 px-2 sm:px-4 md:px-8 py-4 sm:py-6">
      <div className="max-w-screen-2xl mx-auto">
        <nav className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-center bg-gray-800 px-3 sm:px-6 py-3 sm:py-4 rounded-xl mb-5 sm:mb-7 shadow-md w-full">
          <Link to="/admin-dashboard" className="text-sky-300 no-underline font-bold text-base md:text-[1.1rem] transition-colors duration-200 px-2 py-1 rounded hover:text-white hover:bg-gray-900">Admin Dashboard</Link>
          <Button onClick={handleLogout} variant="secondary">Logout</Button>
        </nav>
        
        <header className="flex flex-col sm:flex-row justify-between items-center mb-6 sm:mb-8 gap-2 w-full">
          <h1 className="text-xl sm:text-2xl font-bold text-center sm:text-left w-full">
        ...
        <div className="flex flex-col md:flex-row gap-3 sm:gap-5 mb-6 sm:mb-8 w-full">
          <div className="bg-gray-800 text-white rounded-lg px-4 sm:px-6 md:px-8 py-4 sm:py-6 shadow text-center flex-1">
            <h3 className="m-0 mb-2 text-base md:text-[1.1rem] text-sky-300">Total Earned</h3>
            <p className="text-lg sm:text-xl md:text-2xl m-0 font-bold">₹{stats.totalEarned.toLocaleString()}</p>
          </div>
          <div className="bg-gray-800 text-white rounded-lg px-4 sm:px-6 md:px-8 py-4 sm:py-6 shadow text-center flex-1">
            <h3 className="m-0 mb-2 text-base md:text-[1.1rem] text-sky-300">Pending Vehicles</h3>
            <p className="text-lg sm:text-xl md:text-2xl m-0 font-bold">{stats.pendingVehicles}</p>
          </div>
          <div className="bg-gray-800 text-white rounded-lg px-4 sm:px-6 md:px-8 py-4 sm:py-6 shadow text-center flex-1">
            <h3 className="m-0 mb-2 text-base md:text-[1.1rem] text-sky-300">Accepted Vehicles</h3>
            <p className="text-lg sm:text-xl md:text-2xl m-0 font-bold">{stats.acceptedVehicles}</p>
          </div>
        </div>

        <div className="w-full">
          <h2 className="mb-4 sm:mb-5 text-sky-300 text-lg md:text-xl font-semibold">Pending Maintenance Requests</h2>
          {logs.length === 0 ? (
            <p>No pending requests.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {logs.map((log) => (
                <div key={log._id} className="bg-gray-800 p-4 rounded-lg shadow text-gray-100">
                  ...
                  {log.description && (
                    <p className="text-sm text-gray-400 mb-3">{log.description}</p>
                  )}
      ...
      {showAcceptModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg w-full max-w-md">
            <h3 className="text-xl font-bold mb-4">Accept Maintenance Request</h3>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg w-full max-w-md">
            <h3 className="text-xl font-bold mb-4">Reject Maintenance Request</h3>
            <div>
